...
export const useAuth = () => {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (responsePayload) => {
      ...
      // Simpan ke Zustand Store (Client State) - ini otomatis tersimpan ke localStorage via persist middleware
      setAuth(token, user);
      ...
      // Cukup pindahkan ke Home, biarkan Zustand Handle reaktivitas datanya
      router.push('/');
    },
    onError: (error: AxiosError<ApiErrorResponse>) => {
      const serverMessage = error.response?.data?.message;
